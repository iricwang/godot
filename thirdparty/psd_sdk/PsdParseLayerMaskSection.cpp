...
// ---------------------------------------------------------------------------------------------------------------------
// Type Tool (text layer) parsing helpers. The PSD format stores text layer content and styling in a 'TySh' block,
// whose body is a PSD "descriptor". The text string lives in the 'Txt ' key (TEXT/unicode string) and the styling
// (font size, fill color) lives in the 'EngineData' key (tdta/raw data). These helpers parse just enough to extract
// that information while precisely consuming every OSType value so the surrounding block stays in sync.
// ---------------------------------------------------------------------------------------------------------------------
namespace
{
	...
	// forward declarations (ParseDescriptor and ConsumeOSTypeValue are mutually recursive)
	bool ParseDescriptor(SyncFileReader& reader, Allocator* allocator, Layer* layer, unsigned int depth);
	bool ConsumeOSTypeValue(SyncFileReader& reader, Allocator* allocator, Layer* layer, unsigned int depth, uint32_t ostype, CaptureMode capture);

	// Reads a PSD descriptor Unicode string: uint32 char-count followed by char-count UTF-16BE code units.
	// Allocates a null-terminated UTF-16 buffer and returns it (caller frees), or nullptr for an empty string.
	uint16_t* ReadUnicodeString(SyncFileReader& reader, Allocator* allocator)
	{
		const uint32_t count = fileUtil::ReadFromFileBE<uint32_t>(reader);
		if (count == 0u)
		{
			return nullptr;
		}

		uint16_t* str = memoryUtil::AllocateArray<uint16_t>(allocator, count + 1u);
		for (uint32_t c = 0u; c < count; ++c)
		{
			str[c] = fileUtil::ReadFromFileBE<uint16_t>(reader);
		}
		str[count] = 0u;
		return str;
	}

	void SkipUnicodeString(SyncFileReader& reader)
	{
		const uint32_t count = fileUtil::ReadFromFileBE<uint32_t>(reader);
		reader.Skip(count * sizeof(uint16_t));
	}

	// Skips a descriptor "key" or "classID": uint32 length, then a 4-byte code if length is 0, else "length" bytes.
	void SkipKeyOrClassId(SyncFileReader& reader)
	{
		const uint32_t len = fileUtil::ReadFromFileBE<uint32_t>(reader);
		reader.Skip((len == 0u) ? 4u : len);
	}

	// Reads a descriptor item key and classifies it as one of the keys we care about.
	SpecialKey ReadDescriptorKey(SyncFileReader& reader)
	{
		const uint32_t keyLen = fileUtil::ReadFromFileBE<uint32_t>(reader);
		if (keyLen == 0u)
		{
			const uint32_t code = fileUtil::ReadFromFileBE<uint32_t>(reader);
			if (code == util::Key<'T', 'x', 't', ' '>::VALUE)
			{
				return SpecialKey::TEXT;
			}
			return SpecialKey::OTHER;
		}

		char buffer[64];
		const uint32_t n = (keyLen < 63u) ? keyLen : 63u;
		reader.Read(buffer, n);
		buffer[n] = '\0';
		if (keyLen > n)
		{
			reader.Skip(keyLen - n);
		}

		if (keyLen == 10u && std::strcmp(buffer, "EngineData") == 0)
		{
			return SpecialKey::ENGINE_DATA;
		}
		return SpecialKey::OTHER;
	}

	// ----- minimal EngineData scanner ------------------------------------------------------------------------------
	// EngineData is a textual, PostScript-like token stream. Structural tokens such as "/FontSize", "/FillColor" and
	// "/Values" are plain ASCII (only parenthesized string literals contain UTF-16 / null bytes), so byte-wise
	// searching for those ASCII tokens is reliable.
	int FindBytes(const uint8_t* hay, uint32_t hayLen, const char* needle, uint32_t from)
	{
		const uint32_t needleLen = static_cast<uint32_t>(std::strlen(needle));
		if (needleLen == 0u || hayLen < needleLen)
		{
			return -1;
		}

		for (uint32_t i = from; i + needleLen <= hayLen; ++i)
		{
			uint32_t j = 0u;
			for (; j < needleLen; ++j)
			{
	...
	double ParseNumberAt(const uint8_t* buf, uint32_t len, uint32_t& pos)
	{
		...
		num[n] = '\0';
		return (n == 0u) ? 0.0 : std::atof(num);
	}

	void ParseEngineData(const uint8_t* buf, uint32_t len, Layer* layer)
	{
		// font size
		const int fontSizePos = FindBytes(buf, len, "/FontSize", 0u);
		if (fontSizePos >= 0)
		{
			uint32_t pos = static_cast<uint32_t>(fontSizePos) + 9u; // strlen("/FontSize")
			const double sz = ParseNumberAt(buf, len, pos);
			if (sz > 0.0)
			{
				layer->fontSize = sz;
			}
		...
		// fill color: "/FillColor << ... /Values [ a r g b ] >>" (Photoshop stores components as ARGB, 0..1)
		const int fillColorPos = FindBytes(buf, len, "/FillColor", 0u);
		if (fillColorPos >= 0)
		{
			const int valuesPos = FindBytes(buf, len, "/Values", static_cast<uint32_t>(fillColorPos));
			if (valuesPos >= 0)
			{
				const int bracketPos = FindBytes(buf, len, "[", static_cast<uint32_t>(valuesPos));
				if (bracketPos >= 0)
				{
					uint32_t pos = static_cast<uint32_t>(bracketPos) + 1u;
					double comp[4] = { 1.0, 0.0, 0.0, 0.0 };
					for (int k = 0; k < 4; ++k)
					{
						comp[k] = ParseNumberAt(buf, len, pos);
					}
					layer->fontColor[0] = static_cast<float>(comp[1]); // R
					layer->fontColor[1] = static_cast<float>(comp[2]); // G
					layer->fontColor[2] = static_cast<float>(comp[3]); // B
					layer->fontColor[3] = static_cast<float>(comp[0]); // A
				}
			}
		}
	}
	...
	bool ConsumeOSTypeValue(SyncFileReader& reader, Allocator* allocator, Layer* layer, unsigned int depth, uint32_t ostype, CaptureMode capture)
	{
		if (ostype == util::Key<'T', 'E', 'X', 'T'>::VALUE)
		{
			if (capture == CaptureMode::TEXT && layer->textContent == nullptr)
			{
				layer->textContent = ReadUnicodeString(reader, allocator);
				layer->isText = true;
			}
			else
			{
				SkipUnicodeString(reader);
			}
			return true;
		}
		else if (ostype == util::Key<'t', 'd', 't', 'a'>::VALUE)
		{
			const uint32_t dataLen = fileUtil::ReadFromFileBE<uint32_t>(reader);
			if (capture == CaptureMode::ENGINE_DATA && dataLen > 0u && dataLen < (64u * 1024u * 1024u))
			{
				uint8_t* raw = memoryUtil::AllocateArray<uint8_t>(allocator, dataLen);
				reader.Read(raw, dataLen);
				ParseEngineData(raw, dataLen, layer);
				memoryUtil::FreeArray(allocator, raw);
			}
			else
			{
				reader.Skip(dataLen);
		...
		else if (ostype == util::Key<'d', 'o', 'u', 'b'>::VALUE)
		{
			reader.Skip(8u);
			return true;
		}
		else if (ostype == util::Key<'U', 'n', 't', 'F'>::VALUE)
		{
			reader.Skip(4u + 8u); // unit key + double
			return true;
		}
		else if (ostype == util::Key<'l', 'o', 'n', 'g'>::VALUE)
		{
			reader.Skip(4u);
			return true;
		}
		else if (ostype == util::Key<'b', 'o', 'o', 'l'>::VALUE)
		{
			reader.Skip(1u);
			return true;
		}
		else if (ostype == util::Key<'e', 'n', 'u', 'm'>::VALUE)
		{
			SkipKeyOrClassId(reader); // type
			SkipKeyOrClassId(reader); // enum value
			return true;
		}
		else if (ostype == util::Key<'t', 'y', 'p', 'e'>::VALUE || ostype == util::Key<'G', 'l', 'b', 'C'>::VALUE)
		{
			SkipUnicodeString(reader); // name from classID
			SkipKeyOrClassId(reader);  // classID
			return true;
		}
		else if (ostype == util::Key<'O', 'b', 'j', 'c'>::VALUE || ostype == util::Key<'G', 'l', 'b', 'O'>::VALUE)
		{
			return ParseDescriptor(reader, allocator, layer, depth + 1u);
		}
		else if (ostype == util::Key<'V', 'l', 'L', 's'>::VALUE)
		{
			const uint32_t count = fileUtil::ReadFromFileBE<uint32_t>(reader);
			if (count > 65535u)
			{
			...
			{
				const uint32_t itemType = fileUtil::ReadFromFileBE<uint32_t>(reader);
				if (!ConsumeOSTypeValue(reader, allocator, layer, depth, itemType, CaptureMode::NONE))
				{
					return false;
				}
			}
		...
		else if (ostype == util::Key<'a', 'l', 'i', 's'>::VALUE)
		{
			const uint32_t dataLen = fileUtil::ReadFromFileBE<uint32_t>(reader);
			reader.Skip(dataLen);
			return true;
		...
		// 'obj ' (Reference) and any unknown OSType have lengths we cannot determine safely; bail out and let the
		// caller resync to the end of the 'TySh' block.
		return false;
	}
	...
	bool ParseDescriptor(SyncFileReader& reader, Allocator* allocator, Layer* layer, unsigned int depth)
	{
		if (depth > 16u)
		{
			return false; // guard against runaway recursion on malformed data
		}

		SkipUnicodeString(reader); // name from classID
		SkipKeyOrClassId(reader);  // classID

		const uint32_t itemCount = fileUtil::ReadFromFileBE<uint32_t>(reader);
		if (itemCount > 65535u)
		{
			return false; // sanity guard against misalignment
		}

		for (uint32_t i = 0u; i < itemCount; ++i)
		{
			const SpecialKey special = ReadDescriptorKey(reader);
			const uint32_t ostype = fileUtil::ReadFromFileBE<uint32_t>(reader);

			CaptureMode capture = CaptureMode::NONE;
			if (special == SpecialKey::TEXT)
			{
				capture = CaptureMode::TEXT;
			}
			else if (special == SpecialKey::ENGINE_DATA)
			{
				capture = CaptureMode::ENGINE_DATA;
			}

			if (!ConsumeOSTypeValue(reader, allocator, layer, depth, ostype, capture))
			{
				return false;
			}
		}
	...
	// Parses the body of a 'TySh' (Type Tool Object Setting) block to extract text content and styling.
	void ParseTypeToolObjectSetting(SyncFileReader& reader, Allocator* allocator, Layer* layer)
	{
		fileUtil::ReadFromFileBE<uint16_t>(reader); // version (== 1)
		for (int i = 0; i < 6; ++i)
		{
			fileUtil::ReadFromFileBE<double>(reader); // transform matrix (xx, xy, yx, yy, tx, ty)
		}
		fileUtil::ReadFromFileBE<uint16_t>(reader); // text version (== 50)
		fileUtil::ReadFromFileBE<uint32_t>(reader); // descriptor version (== 16)

		// the text descriptor holds 'Txt ' (content) and 'EngineData' (styling).
		ParseDescriptor(reader, allocator, layer, 0u);

		// the trailing warp descriptor + coordinates are intentionally left unread; the caller resyncs to the
	...
	static LayerMaskSection* ParseLayer(const Document* document, SyncFileReader& reader, Allocator* allocator, uint64_t sectionOffset, uint32_t sectionLength, uint32_t layerLength)
	{
		...
		if (layerLength != 0)
		{
			...
			for (unsigned int i=0; i < layerMaskSection->layerCount; ++i)
			{
				...
				layer->textContent = nullptr;
				layer->smartObjectPath = nullptr;
				layer->isText = false;
				layer->fontSize = 0.0;
				layer->fontColor[0] = 0.0f;
				layer->fontColor[1] = 0.0f;
				layer->fontColor[2] = 0.0f;
				layer->fontColor[3] = 1.0f;
				layer->fontName = nullptr;
				...
				while (toRead > 0)
				{
					...
					// read Type Tool Object Setting (Photoshop 6.0+) to extract text layer content and styling
					else if (key == util::Key<'T', 'y', 'S', 'h'>::VALUE || key == util::Key<'t', 'y', 'S', 'h'>::VALUE)
					{
						const uint64_t blockStart = reader.GetPosition();
						ParseTypeToolObjectSetting(reader, allocator, layer);

						// the descriptor is variable-length, so resync to the declared block end to keep "toRead" correct.
						const uint64_t consumed = reader.GetPosition() - blockStart;
						if (consumed < length)
						{
						...
						{
							// parsing overran the block (corrupt/unexpected data); reposition to the known end.
							reader.SetPosition(blockStart + length);
						}
					}
					else
					{
...
void DestroyLayerMaskSection(LayerMaskSection*& section, Allocator* allocator)
{
	...
	for (unsigned int i=0; i < section->layerCount; ++i)
	{
		...
		memoryUtil::FreeArray(allocator, layer->textContent);
		memoryUtil::FreeArray(allocator, layer->smartObjectPath);
		memoryUtil::FreeArray(allocator, layer->fontName);
